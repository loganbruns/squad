...
# High-level options
tf.app.flags._global_parser = argparse.ArgumentParser(conflict_handler='resolve')
...
def main(unused_argv):
    ...
    # Define train_dir
    if not FLAGS.experiment_name and not FLAGS.train_dir and (FLAGS.mode != "official_eval" and FLAGS.mode != "old_official_eval"):
    ...
    if FLAGS.mode == "train":
        ...
        answers_dict_by_model = {}

        models = ['best_checkpoint_v18', 'best_checkpoint_v18_2', 'best_checkpoint_v18_3', 'best_checkpoint_v18_4', 'best_checkpoint_v18_5']
        # models = ['v18', 'v18_2', 'v18_3', 'v18_4']
        for model in models:
            ...
            with tf.Session(config=config) as sess:
                ...
                print 'Evaluate using {} model'.format(model)
                # initialize_model(sess, qa_model, 'experiments/{}/best_checkpoint'.format(model), expect_exists=True)
                initialize_model(sess, qa_model, model, expect_exists=True)

                # Get a predicted answer for each example in the data
                # Return a mapping answers_dict from uuid to answer
                answers_dict_by_model[model] = generate_answers(sess, qa_model, word2id, qn_uuid_data, context_token_data, qn_token_data)

        # tf.app.flags.DEFINE_integer("embedding_size", 100, "Size of the pretrained word vectors. This needs to be one of the available GloVe dimensions: 50/100/200/300")
        # emb_matrix, word2id, id2word = get_glove(FLAGS.glove_path, FLAGS.embedding_size)
        # models = ['v14_1', 'v20', 'v20_1', 'v20_2']
        # for model in models:
        #     # Read the JSON data from file
        #     qn_uuid_data, context_token_data, qn_token_data = get_json_data(FLAGS.json_in_path)
        ...
        answers_dict = {}
        for uid in answers_dict_by_model[models[0]].keys():
            values = [answers_dict_by_model[model][uid] for model in models]
            answers_dict[uid] = sorted([(len(list(group)), key) for key, group in groupby(values)], reverse=True)[0][1]
    ...
    elif FLAGS.mode == "old_official_eval":
        if FLAGS.json_in_path == "":
            raise Exception("For official_eval mode, you need to specify --json_in_path")
